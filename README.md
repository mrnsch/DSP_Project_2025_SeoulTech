...
The core research question is: **Does the frequency of unethical language correlate with a media's influence, and can we identify distinct media archetypes based on a two-dimensional space (Bias Score, Influence Score)?**
...
The analysis was executed through a rigorous three-phase pipeline, systematically comparing nine model combinations (3 Dictionary Methods $\times$ 3 Clustering Algorithms) to ensure the stability and validity of the final structure.

### Phase 1: Bias Dictionary Construction (NLP Feature Engineering)
...
| :--- | :--- |
| `Project_DSP_methodABC.ipynb` | Initial data cleaning, feature engineering, and generation of the three Bias Scores (Methods A, B, C) and Influence Scores. |
| `Dictionary_And_Clustering_Comparison.ipynb` | Systematic execution of the **9 model combinations** and storage of all evaluation metrics in `comparison_metrics.csv`. |
...
| `comparison_metrics.csv` | Output file containing Silhouette scores and other metrics for all 9 model trials. |
